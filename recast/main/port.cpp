...
namespace demo {
    ...
    void buildNavmesh(const FunctionCallbackInfo<Value>& args) {
        ...
        float c_cellSize = Local<v8::Number>::Cast(args[0])->NumberValue(context).FromJust();
        float c_cellHeight = Local<v8::Number>::Cast(args[1])->NumberValue(context).FromJust();
        float c_agentHeight = Local<v8::Number>::Cast(args[2])->NumberValue(context).FromJust();
        float c_agentRadius = Local<v8::Number>::Cast(args[3])->NumberValue(context).FromJust();
        float c_agentMaxClimp = Local<v8::Number>::Cast(args[4])->NumberValue(context).FromJust();
        float c_agentMaxSlope = Local<v8::Number>::Cast(args[5])->NumberValue(context).FromJust();
        float c_regionMinSize = Local<v8::Number>::Cast(args[6])->NumberValue(context).FromJust();
        float c_regionMergeSize = Local<v8::Number>::Cast(args[7])->NumberValue(context).FromJust();
        float c_edgeMaxLen = Local<v8::Number>::Cast(args[8])->NumberValue(context).FromJust();
        float c_edgeMaxError = Local<v8::Number>::Cast(args[9])->NumberValue(context).FromJust();
        float c_vertsPerPoly = Local<v8::Number>::Cast(args[10])->NumberValue(context).FromJust();
        float c_detailSampleDist = Local<v8::Number>::Cast(args[11])->NumberValue(context).FromJust();
        float c_detailSampleMaxError = Local<v8::Number>::Cast(args[12])->NumberValue(context).FromJust();
        ...
        Local<String> n_mesh = v8::String::NewFromUtf8(isolate, result).ToLocalChecked();
